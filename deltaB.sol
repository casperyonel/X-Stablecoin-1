...
// Libraries
import "contracts/Libraries/LibMetaCurve.sol";
import "contracts/Libraries/LibMetaCurve.sol";

// Contracts
import {P} from "contracts/P.sol";
...
contract deltaB {
    ...
    //-------------------------------------------------------------------------------------------------------------------
    // Mainnet
    address public constant POOL = 0xc9C32cd16Bf7eFB85Ff14e0c8603cc90F6F2eE49; // REPLACE, CREATE NEW POOL, i changed from pivate to public btw
    address private constant CRV3_POOL = 0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7; 
    address private constant BEANSTALK = 0xC1E088fC1323b20BCBee9bd1B9fC9546db5624C5; // DELETE
    //-------------------------------------------------------------------------------------------------------------------
    ...
    address[2] private tokens = [0xBEA0000029AD1c77D3d5D23Ba2D8893dB9d1Efab, 0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490]; // REPLACE WITH VIBE TOKNE ADDRESS, 3CRV
    ...
    function getCurve() public view returns (P.Pool memory pool) {
        ...
        pool.lpBdv = IBDV(BEANSTALK).bdv(POOL, 1e18); // DELETE
    }

    function getCurveDeltaB(uint256 balance, uint256 D) private pure returns (int XdeltaB) {
        uint256 pegBeans = D / 2 / 1e12;
        XdeltaB = int256(pegBeans) - int256(balance);
    }
